...
public class LoginHandler : IHandler
{
    ...
    public async Task<JwtSecurityToken> LoginUserAsync(LoginModel model)
    {
        var user = await _userManager.FindByNameAsync(model.Username);

        if (user != null && user.IsDeleted == true)
        {
            throw new InternalException("This account has been deleted, please use a different account.", 404, HttpStatusCode.NotFound);
        }
        ...
        throw new InternalException("Invalid credentials", 401, HttpStatusCode.Unauthorized);
    }
}
